...
public class MysqlAppTest {
  ...
  public void json() {
    ...
    Assertions.assertThat(r.getBody())
        .isEqualTo(
            "{\"a1\":\"10\",\"bb\":\"扯淡扯扯淡\",\"password\":\"世界上本没有密码，用的人多了，也就有了\","
                + "\"c1\":\"123456789012567890123458901256789012346789012345890123467890123456"
                + "789012567890123458901256789012346789012345890123467890\","
                + "\"b1\":\"毛主席在天安门城楼上宣布中华人民共和国中央人民政府今天成立了\"}");
  ...
  public void fork() {
    ...
    ResponseEntity<String> response =
        restTemplate.exchange(
            createURLWithPort("/test/fork"), HttpMethod.GET, entity, String.class);
